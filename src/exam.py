...
class Exam:
    ...
    def make_docx(ids: list, title: str):
        ...
        doc.styles["Normal"].font.name = "Cambria"  # type: ignore
        doc.styles["Normal"]._element.rPr.rFonts.set(qn("w:eastAsia"), "SimSun-ExtB")  # type: ignore
        ...
        for i, q in enumerate(Exam.search_for(ids)):
            ...
            if q["questionType"] in ("radio", "checkbox"):
                for opt in q["options"]:
                    doc.add_paragraph(opt["option"] + ". " + opt["content"].strip())
